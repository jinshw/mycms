...
router.post("/publishColumn", function(req, res, next) {
  articleDao.getByConditions({columnid:req.body.columnid}).then(function(data) {
    var _result = {};
    _result.data = data;

    // 发布列表
    var _content = fs.readFileSync(
      config.TEMPLATE_URL + req.body.templatelist,
      "utf-8"
    );
    ...
    fs.writeFileSync(_columnPath + "/" + req.body.columnid + ".html", html);

    // 发布文章
    for (var index in data) {
      if (data[index]._doc.link == null) {
        var _contentArt = fs.readFileSync(
          config.TEMPLATE_URL + req.body.templatedetail,
          "utf-8"
        );
        // var _contentArt = fs.readFileSync(config.TEMPLATE_URL + "templ.ejs", "utf-8")
        let html = ejs.render(_contentArt, {
          data: data[index],
          baseurl: config.HTML_BASEURL
        });
        fs.writeFileSync(_columnPath + "/" + data[index]._id + ".html", html);
...
router.get("/getConfig", function(req, res, next) {
  var result = { id: 123, name: "jinshw" };
  return res.end(JSON.stringify(config.ARTICLE_CONFIG));
});
